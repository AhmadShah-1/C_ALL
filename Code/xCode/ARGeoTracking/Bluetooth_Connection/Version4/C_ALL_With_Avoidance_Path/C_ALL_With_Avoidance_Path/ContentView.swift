...
struct ContentView: View {
    ...
    @State private var showVoiceInputPrompt: Bool = true // Show prompt at launch
    ...
    var body: some View {
            ...
            if showVoiceInputPrompt {
                VStack {
                    ...
                    VStack(spacing: 16) {
                        ...
                        Text("Would you like to specify your destination by voice?")
        ...
        .sheet(isPresented: $showVoiceInput) {
            VoiceInputView(
                ...
                onLocationSelected: processVoiceDestination
            )
        }
    ...
    func processVoiceDestination(_ coordinate: CLLocationCoordinate2D) {
        selectedCoordinate = coordinate
        
        // Create the route
        if let userLocation = locationManager.location?.coordinate {
            setupNavigationRoute(from: userLocation, to: coordinate)
        }
        
        // Track that we've processed a voice destination
        UserDefaults.standard.set(false, forKey: "HasSiriDestination")
    }
    
    // Process Siri requests from app launch
    func processSiriRequestIfNeeded() {
        NotificationCenter.default.addObserver(forName: Notification.Name("SiriSelectedDestination"), object: nil, queue: .main) { notification in
            if let userInfo = notification.userInfo,
               let coordinate = userInfo["coordinate"] as? CLLocationCoordinate2D {
                DispatchQueue.main.async {
                    self.processVoiceDestination(coordinate)
                }
            }
        }
    }
